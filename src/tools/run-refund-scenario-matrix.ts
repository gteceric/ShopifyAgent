  ...
  const results = await Promise.all(
    REFUND_SCENARIO_MATRIX.map(async (scenario) => {
      ...
      return {
        ...
        actualDecision: response.result.policyResult.decision,
        matchedDecision: response.result.policyResult.decision === scenario.expectedDecision,
        expectedReasonCodes: scenario.expectedReasonCodes,
        actualReasonCodes: response.result.policyResult.reasons.map((reason) => reason.code),
